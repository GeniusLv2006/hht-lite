...
本项目最初面向有限范围的个人使用场景开发。现有的数据记录与管理功能源于当时的运营需要，并未按通用公开部署或跨司法管辖区运营场景设计。任何公开部署均应重新评估数据收集的必要性与范围，并优先采用数据最小化方案。

本项目不是 GDPR 或 UK GDPR 合规模板，也未接受相关法律或合规审查。部署和运营本项目可能涉及 OpenID、IP 地址、User-Agent、访问日志及其他个人数据的处理。部署者应根据其所在地、目标用户、基础设施、第三方服务和实际数据处理活动，独立确定适用法律，并落实相应的数据保护评估、隐私告知以及技术和组织措施。
